...
app=Flask(__name__); app.secret_key=os.environ.get('PUBAFRICA_SECRET','change-me'); DB='pubafrica1.db'
...
def login():
 if request.method=='POST':
  ...
  if u and check_password_hash(u['password'],request.form['password']): session['uid']=u['id'];session['name']=u['name'];return redirect('/dashboard')
...
def publish():
 if request.method=='POST':
  c=db();c.execute('INSERT INTO listings(user_id,title,description,category,location,price) VALUES(?,?,?,?,?,?)',(session['uid'],request.form['title'],request.form['description'],request.form['category'],request.form['location'],request.form['price']));c.commit();flash('Annonce envoyée pour validation.');return redirect('/dashboard')
 return page('''<div class=panel><h1>Publier gratuitement</h1><form method=post><input name=title placeholder="Titre" required><textarea name=description placeholder="Description" required></textarea><input name=category placeholder="Catégorie" required><input name=location placeholder="Pays / ville" required><input name=price placeholder="Prix"><button>Envoyer</button></form></div>''')
@app.route('/listing/<int:i>')
def listing(i):
 x=db().execute("SELECT l.*,u.name FROM listings l JOIN users u ON u.id=l.user_id WHERE l.id=? AND l.status='published'",(i,)).fetchone(); return page('''<div class=panel>{% if x %}<p class=muted>{{x.category}} · {{x.location}}</p><h1>{{x.title}}</h1><p>{{x.description}}</p><h2>{{x.price or 'Prix sur demande'}}</h2><p>Annonceur : {{x.name}}</p>{% else %}<h1>Annonce introuvable</h1>{% endif %}</div>''',x=x)
